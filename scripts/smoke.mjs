...
try{
 ...
 await w.locator('#manual-numbers').fill('1 2 3 4 5 6');await w.locator('#manual-bonus').fill('2');await w.locator('#manual-date').fill('2026-09-07');await w.locator('#manual-form button').click();await w.waitForFunction(()=>document.querySelectorAll('[data-gain]').length===2);
 assert.equal(JSON.parse(await readFile(path.join(dir,'predictions.json'),'utf8')).tickets.filter(t=>t.method==='manual').length,1);
 await w.locator('#manual-numbers').fill('1 1 3 4 5 6');await w.locator('#manual-bonus').fill('2');await w.locator('#manual-form button').click();await w.waitForFunction(()=>document.querySelector('#toast')?.textContent.includes('Combinaison invalide'));assert.equal(await w.locator('[data-gain]').count(),2);
 await w.getByRole('button',{name:'Sources & mises à jour',exact:false}).click();await w.locator('#notify-results').check();await w.waitForFunction(async()=> (await window.desktop.state()).value.notifications?.results===true);await w.locator('#notify-minutes').selectOption('60');await w.waitForFunction(async()=> (await window.desktop.state()).value.notifications?.minutes===60);
 ...
 if(!packaged){await w.getByRole('button',{name:'Sources & mises à jour',exact:false}).click();await w.locator('#check-update').click();await w.waitForFunction(()=>document.querySelector('#update-status')?.textContent.includes('version installée'));}
 await app.close();app=await launch();w=await app.firstWindow();await w.waitForSelector('.heatmap');await w.getByRole('button',{name:'EuroDreams',exact:true}).click();await w.getByRole('button',{name:'Mes grilles'}).click();await w.waitForSelector('[data-gain]');assert.equal(await w.locator('[data-gain]').first().inputValue(),'0');
 assert.equal(await w.locator('[data-gain]').count(),2);await w.getByRole('button',{name:'Sources & mises à jour',exact:false}).click();assert.equal(await w.locator('#notify-results').isChecked(),true);assert.equal(await w.locator('#notify-minutes').inputValue(),'60');
